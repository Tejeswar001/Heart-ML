...
export default function HospitalDashboardPage() {
  ...
  const downloadResults = () => {
    ...
    const csvRows = filteredPatients
      .map(
        (p) =>
          `${p.id},${p.age},${p.gender},${p.riskLevel},${(
            p.probability * 100
          ).toFixed(1)}%`
      )
      .join("\n");
    ...
    a.href = url;
    a.download = `cardio_risk_results_${
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground text-lg">
            Patient cardiovascular risk overview and analysis
          </p>
        </div>
                  ...
                  <TableBody>
                    {filteredPatients.map((patient) => (
                      <TableRow
                        key={patient.id}
                        className={
                          patient.riskLevel === "high" ? "bg-destructive/5" : ""
                        }
                      >
        ...
        {stats.high > 0 && (
          <Card className="mt-6 bg-destructive/5 border-destructive/20">
            ...
            <CardContent className="text-sm text-muted-foreground leading-relaxed">
              <p>
                You have{" "}
                <strong className="text-destructive">{stats.high}</strong>{" "}
                patient(s) identified with high cardiovascular risk. We
                recommend prioritizing follow-up consultations and additional
